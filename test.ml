...
let test13c test_ctxt = assert_equal ~printer:(fun p -> Printf.sprintf "%s" p) 
  (stringFromZExp (fst arrowSelectedParent))
  (stringFromZExp (fst (performSyn arrowSelectedRight (Move Parent))))


(* let test13d test_ctxt = assert_equal ~printer:(fun p -> Printf.sprintf "%s" p) 
  (stringFromZExp (fst arrowSelectedRight))
  (stringFromZExp (fst (performSyn arrowSelectedParent (Move Parent))))
 *)
